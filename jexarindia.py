...
#-------------------------------------------------------------#
            ...
            if "1200" in data.hex()[0:4] and b"BEM VINDO A [FFFFF00]LADY BUG [ffffff]BOT" in data:
	            ...
	            if "1200" in data.hex()[0:4] and b"/ghost" in data:
	                ...
	                command_split = re.split("/ghost", str(data))
	                if len(command_split) > 1:
	                    squad_id = command_split[1].split('(')[0].strip()

	                    print(f"[C][B]Iniciando ghost para o squad {squad_id}")
	                    json_result = get_available_room(data.hex()[10:])
	                    parsed_data = json.loads(json_result)
	                    uid = parsed_data["5"]["data"]["1"]["data"]
	                    clients.send(
            self.GenResponsMsg(
                f"{generate_random_color()}Iniciando ghost para o squad {squad_id}...", uid
	                    )
	                )
	                    
	                    message = self.ghost(
    player_id=squad_id,
    secret_code="LADY BUG",  # substitua pelo valor correto
    key=self.key,
    iv=self.iv
)
	                    json_result = get_available_room(data.hex()[10:])
...
[C][B][00FF00]/🤔3 -> /🤔4 -> /🤔5 -> /🤔6
[C][B][FFFFFF]limite de pessoas no squad""", uid
	                    )
	                )
	                time.sleep(0.5)
	                clients.send(
